...
<link rel="stylesheet" type="text/css" href="skin/css/base.css">
    ...
    <script type="text/javascript">
        $(function () {
            $(".statusClass").each(function (index,itme) {
                var startTime=$(this).parent().find("td:eq(6)").text();
                var endTime=$(this).parent().find("td:eq(7)").text();

                startTime=new Date(startTime);
                var timestamp01=startTime.getTime();

                endTime=new Date(endTime);
                var timestamp02=endTime.getTime();

                var now=Date.parse(new Date());

                if(timestamp01-now>0){
                    $(this).text("未开始...");
                }
                if(now-timestamp01>0&&now-timestamp02<0){
                    $(this).text("进行中...");
                }
                if(now-timestamp01>0&&now-timestamp02>0){
                    $(this).text("已完成...");
...
<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
	...
	<td class="statusClass" align="center"></td>
